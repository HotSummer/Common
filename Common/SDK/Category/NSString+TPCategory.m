...
-(CGSize)textSize:(UIFont*)f withWidth:(CGFloat)w{
    return MULTILINE_TEXTSIZE(self, f, CGSizeMake(w, CGFLOAT_MAX), NSLineBreakByWordWrapping);
}
...
- (CGFloat)widthwithfont:(UIFont *)font height:(CGFloat)height
{
    ...
    CGSize labelSize = MULTILINE_TEXTSIZE(self, font, constraintSize, NSLineBreakByWordWrapping);
    
	return labelSize.width;
}

- (CGSize)heightWithFont:(UIFont*)font constrainedToSize:(CGSize)size
{
    CGSize labelSize = MULTILINE_TEXTSIZE(self, font, size, NSLineBreakByWordWrapping);
...
-(NSString*)secretName{
    ...
    if (self.length>3) {
        self = [self stringByReplacingCharactersInRange:NSMakeRange(2, self.length-3) withString:@""];
    }
    
    self = [self stringByReplacingCharactersInRange:NSMakeRange(1, self.length-1) withString:@"**"];
